...
const wordsToFind = ['rosu', 'caine', 'alb', 'ras', 'mov'];
let size = 9;
...
function checkSelectedWord() {
    ...
    if (selectedWord) {
        for (let i in wordsToFind) {
            if (selectedWord === wordsToFind[i]) {
                ...
                if(isOnSameLine) {
                    ...
                    if (dialogContainer) {
                        dialogContainer.innerHTML = 'Cuvantul nu este unul bun! Cuvintele selectate trebuie sa fie pe aceeasi linie, coloana sau diagonala!' + '<br>' + 'Cuvintele cautate sunt:  rosu, caine, alb, ras, mov';
                    }
                    clearSelection();
                }
            }
        ...
        if (wordsToFind.length === 0) {
            let dialogContainer = document.getElementById('dialog-container');
            if (dialogContainer) {
                dialogContainer.innerHTML = '<h2>Yeeeee - Haaaa</h2>' + '<p>Felicitari! Esti un adevarat geniu al cuvintelor!</p>' + '<p>Ai reusit sa distrugi si nivelul cel mai greu!</p>' + '<p>Pentru aceasta izbanda meriti o comoara!</p>' + '<button id="claimTreasureButton">Revendică Comoara</button>';

                var claimTreasureButton = document.getElementById('claimTreasureButton');
                if (claimTreasureButton) {
                    claimTreasureButton.onclick = function () {
                        window.location.href = '../paginaFInala/paginaFinala.html';
                    };
                }
            }
        } else {
            ...
            if (!found) {
                ...
                if (nr === wordsToFind.length) {
                    let dialogContainer = document.getElementById('dialog-container');
                    if (dialogContainer) {
                        dialogContainer.innerHTML = 'Cuvantul nu este unul bun! Incearca din nou!' + '<br>' + 'Cuvintele cautate sunt:  rosu, caine, alb, ras, mov';
                    }
                    clearSelection();
                }
            }
