...
from automate.AF import AF

def definire_af(filename):
    tranzitii = []
    ...
    automat = AF(stari_initiale, alfabet, tranzitii, stari_finale)
    return automat

af_nr_intregi = definire_af("automate/af_nr_intregi.txt")
af_nr_reale = definire_af("automate/af_nr_reale.txt")
af_variabile = definire_af("automate/af_variabile.txt")
af_string_constant = definire_af("automate/af_string_constant.txt")
...
def classify_token(token, keywords, operators, separators):
    if token in keywords:
        ...
        return "constants"
    elif af_variabile.este_acceptata_seq(token) and len(token)<=8:
        return "identifiers"
    else:
        return "error"
...
def is_constant(token):
        ...
        if(af_nr_reale.este_acceptata_seq(token) or af_nr_intregi.este_acceptata_seq(token) or af_string_constant.este_acceptata_seq(token)):
            return True
        return False
