...
def partitie2(array, start, end,key):
    ...
    while True:
        ...
        if low <= high:
            array[low], array[high] = array[high], array[low]
            print(key(array[low]),key(array[high]))
...
def QuickSort(array, start, end,reverse = False,cmp=False,key=lambda x:x):
    """
...
def test_QuickSort():
    ...
    assert(QuickSort(lista1,0,len(lista)-1,key = lambda x:x.getnume()) == lista_nume)
