...
class note_repository_file(note_repository):
    ...
    def __load_from_file(self):
        ...
        for line in lines:
            student1,labotor1,nota = [token.strip() for token in line.split('/')]
            nota = notare(student1,labotor1,float(nota))
    ...
    def __save_to_file(self):
        ...
        with open(self.__filename, 'w') as f:
            for el in lista:
                string = str(el.getstudent()) + '/' + str(el.getlaborator()) + '/' + str(el.getnota()) + '\n'
                f.write(string)
    def save(self,note):
