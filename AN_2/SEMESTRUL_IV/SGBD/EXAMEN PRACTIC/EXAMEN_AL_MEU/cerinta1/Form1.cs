...
namespace cerinta1
{
    ...
    {
        SqlConnection cs = new SqlConnection("Data Source=DESKTOP-5LTINQ1\\SQLEXPRESS;Initial Catalog=S7;Integrated Security=True"); //
        ...
        {
            da.SelectCommand = new SqlCommand("SELECT * FROM TipuriInghetate", cs);
            ds.Clear();
            da.Fill(ds);
        ...
        {
            da.SelectCommand = new SqlCommand("SELECT * FROM Inghetate WHERE Tid= @idParinte", cs); 
        ...
        private void adaugaBtnClick(object sender, EventArgs e)
        {
            ...
            {
                cs.Open();
                da.InsertCommand = new SqlCommand("Insert into Inghetate(Denumire,Pret,Gramaj,Tid)" + 
                "values(@denumire,@pret,@gramaj,@cod_parinte)", cs); 

                da.InsertCommand.Parameters.AddWithValue("@denumire", textBox1.Text); 
                da.InsertCommand.Parameters.AddWithValue("@pret", textBox2.Text); 
                da.InsertCommand.Parameters.AddWithValue("@gramaj", textBox3.Text); 
        ...
        private void stergeBtnClick(object sender, EventArgs e)
        {
            ...
            {
                cs.Open();
                da.DeleteCommand = new SqlCommand("DELETE FROM Inghetate WHERE Iid = @idCopil", cs); ////
        ...
        private void updateBtnClick(object sender, EventArgs e)
        {
            ...
            {
                cs.Open();
                da.UpdateCommand = new SqlCommand("UPDATE Inghetate SET Denumire=@denumire, Pret=@pret,Gramaj=@gramaj " + 
                "WHERE Iid=@idCopil", cs); 

                da.UpdateCommand.Parameters.AddWithValue("@denumire", textBox1.Text); 
                da.UpdateCommand.Parameters.AddWithValue("@pret", textBox2.Text); 
                da.UpdateCommand.Parameters.AddWithValue("@gramaj", textBox3.Text); 
